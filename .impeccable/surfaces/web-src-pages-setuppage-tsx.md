...
- **兩條路徑的剖面不同**：套件內的剖面列「將會做什麼」（各步驟打哪一支端點），既有的剖面
  列「這台 Jellyfin 現在是什麼樣子」（位址、版本、API key、媒體庫數）。同一個 `Cutaway`
  元件，內容全部是實測值。**票 14b 之後**：序列剩七步（插件與重啟那兩步整段移除），剖面也
  不再有 MergeVersions 那一列；版本低於 12.0 時，工作面最上方多一段紅色的版本閘門說明。
- **二次確認就地展開**：「加入 Berth 路徑」按下之後在原地換成一段說明加兩顆按鈕，不開
  dialog——沒有 Radix，也不跳離當前泊位。（票 14b 之前這裡還有第二顆「安裝 MergeVersions」。）
...
## Resolved during build (票 06d)

- **導覽**（`/impeccable onboard` shape，使用者確認，細節在 `.scratch/m0/wizard-shape.md` §8）：每個泊位做完停在結果上，
  「前往下一個泊位」接手主要動作；泊位板上走過的格子是按鈕；前置兩步住在板上方的前置列（證據即入口）；回頭看有
  「回到目前這一步」與「這裡能做 / 不在這裡做」的說明。原本 trail 的「改帳密」「重新探測」拿掉。
- **Route 移到 qBittorrent 之後**（第 5 步、BTH 3），套件內走到就自動跑，沒有要按的鍵。
- **「重新偵測這個服務」**放在出問題的那一格旁邊，不在每頁頂部。
