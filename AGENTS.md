...
- **Issue tracker**：local markdown。spec 在 `.scratch/<feature-slug>/spec.md`，票在
  `.scratch/<feature-slug>/issues/NN-slug.md`（從 01 起編），狀態在票頂的 `Status:` 行，
  討論記在票尾的 `## Comments`；`/wayfinder` 的地圖是 `.scratch/<effort>/map.md`。
- **Triage labels**：五個預設角色標籤，字串與角色名相同。
- **Domain docs**：根目錄 `CONTEXT.md` 是名詞表的唯一來源，命名一律用它的詞。
