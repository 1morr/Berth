...
const zhHant = {
  ...
  media: {
    ...
    runtime: '片長',
    counts: '季數與集數',
    // 追蹤前後說的是同一串字，但一個是「將會是」一個是「就是」——按下去那一刻它就凍結了。
    folderPreview: '資料夾將會是',
    folder: '資料夾（已凍結）',
    seasons: '季集',
    tracked: '已追蹤',
    isTracked: 'Berth 在管這部作品。資料夾名已經凍結，之後 TMDB 改標題也不會動它。',
    untracked: '還沒交給 Berth 管。追蹤之後才送得了下載，而資料夾名會在那一刻凍結。',
    track: '追蹤',
    retrack: '更新 Route',
    tracking: '寫入中…',
    trackFailed: '寫不進去。Berth 後端可能沒在跑——確認之後再按一次。',
    backToDiscover: '回探索頁',
    stale: '快照是舊的',
    fetchedAt: '快照抓取於',
    refresh: '立即重抓',
    ...
    route: {
      ...
      missing: {
        tv: '還沒有任何一條收劇集的 Route。沒有它，追蹤了也沒有地方可以入庫。',
        movie: '還沒有任何一條收電影的 Route。沒有它，追蹤了也沒有地方可以入庫。',
      },
    },
  },
...
const en: Translations<typeof zhHant> = {
  ...
  media: {
    ...
    folder: 'Folder (frozen)',
    seasons: 'Seasons',
    tracked: 'Tracked',
    isTracked:
      'Berth manages this title. The folder name is frozen — a later TMDB rename will not move it.',
    untracked:
      'Not managed by Berth yet. Tracking is what makes downloads possible, and it freezes the folder name.',
    track: 'Track',
    retrack: 'Update route',
    tracking: 'Saving…',
    trackFailed: 'That did not go through. The Berth backend may be down — check, then try again.',
    ...
    route: {
      ...
      missing: {
        tv: 'No route accepts series yet. Without one there is nowhere to import a tracked title.',
        movie: 'No route accepts films yet. Without one there is nowhere to import a tracked title.',
      },
    },
  },
  tmdb: {
