...
test('加 Feed → 輪詢 → 綁定待綁定的那一部 → 下載列表上有它的兩集', async ({ page }) => {
  ...
  await expect(feed.getByRole('status')).toHaveText(
    '這一輪：新 12 筆、長出 11 個 RSS Series、送出 0 筆。',
  )
  ...
  await row.getByRole('button', { name: '綁定' }).click()
  await expect(row.getByRole('searchbox')).toHaveValue('Kimi ga Shinu made Koi wo Shitai')
  await row.getByRole('button', { name: /與妳相戀到生命盡頭/ }).click()
