...
**整段確認**（使用者 2026-09-24 試跑時拍板：一包 12 集全是 medium 要逐列按）：「已入庫・等你看一眼」那一段的標題列再一顆「全部確認」，範圍是**畫面上列出的那些**——前端送出它看到的 ledger id，不是伺服器端的「全部」，按下之後才進來的 audit 不會被順手確認掉。先就地確認並說出件數（DESIGN.md 就地確認）；後端與整組確認共用同一個命令，逐列語意、跳過規則相同。只作用於 audit 段：低信心 Plan、Unmatched、重複版本都要逐件判斷，不給整段按鈕。

**收起時說出為什麼是 medium**（使用者 2026-09-24 試跑時拍板）：列收起時只有「信心 medium，已自動入庫」，使用者看不出要看什麼，得展開才看到理由。收起的那一行接上**主要原因**一句，例如「信心 medium：季號是推論的（TMDB 只有一季）」。主要原因從理由 code 挑（`strategy` 不是 `explicit` / `folder` / `context` 的那一條，或讓批次一致性只部分通過的那一條），挑選是純函式；完整理由仍在展開裡。整組（同一個 Job）收起時同理，組內原因相同就說一次。
