...
- **`WizardTrail` 瘦身**：有了導覽之後它只留證據（管理員是誰、判定了幾個服務），「改帳密」回到第 1 步上（泊位板點得到第 1、2 步的話，那兩步也要點得到，shape 決定它們在板上的位置）。
- **「重新探測」放在出問題的那一格旁邊，不是每頁頂部**（使用者 2026-09-25：它仍然需要）：某個服務判定未解決、逾時或那一格的動作連不上時，在那一格、那一頁給「重新偵測這個服務」；全部重新探測留在第 2 步。啟動中的暫時錯誤要人按才好的那一種由 06g 修掉，這顆鍵剩下的用途是使用者自己改了東西之後。
- **只處理第一次設定**（使用者 2026-09-25 拍板，票 06i）：精靈跑完之後的修改由設定頁接手，`/setup` 導向設定頁，`?berth=` 回精靈的那條路在 06i 刪掉。所以這張票的導覽只管「還沒跑完」的精靈，不必處理「跑完之後又回來」的覆寫。
- **回頭看的泊位說出能改什麼**（使用者 2026-09-25 問「加了下一步之後還能改之前的設定嗎」）：每一步都是冪等命令，回頭照樣重跑，做過的標「已經是這樣」。畫面上要說出這一格回頭能做的事與不能在這裡做的事去哪裡做——套件內 Jellyfin 建好的媒體庫改名去 Jellyfin（06f）、Route 的改名與停用去 `/settings/routes`（精靈只新增，M1 票 14）、帳密見 06c。
- **拿掉完成頁的「回媒體庫路徑」**：它是現在唯一的一顆「上一步」，名字卻像一個功能；有了通用的「上一個泊位」之後不需要它。
