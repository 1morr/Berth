...
- **本票的第 2、3、5、7 條驗收由票 04b 取代**（2026-09-09，勾選歷史照原樣留著；票 04b 的驗收
  漏列了第 3 條「refresh 不改 `folder_name`」，那條正是被反過來的那一條）：`media.tracked`
  與 `POST /media/{id}/track` 移除，凍結 `folder_name` 移到票 09 的送單成功那一刻，探索牆的
  卡片不再顯示「已追蹤」。理由是「追蹤」在 `CONTEXT.md` 本來就是**推導出來的結果**而不是一顆
  按鈕，而它在畫面上讀起來像訂閱。下面那幾條 Comments 是本票當時的判斷，其中關於 `tracked`
  與凍結時機的部分已經被推翻。
