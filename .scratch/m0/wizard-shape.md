...
**主要任務**：把五個泊位接上，走到健康頁綠燈（2026-09-25 票 06e 起五個泊位；原本四個）。
...
- **結構論點**：精靈不是八張「下一步」的表單，是一塊**調度板**。泊位橫列常駐（原本四個，票 06e 起五個），工作面在下。
...
| 泊位 4 索引站 | 6（Prowlarr 或 Torznab；加入 → 試搜 → 移除，可以之後再說） |
| 泊位 5 TMDB | 7（必填的閘門，票 06e 從泊位 4 拆出來） |
| （收尾） | 8 完成 |
...
- 服務判定五種：套件內 / 既有 / 探測中 / 逾時（輪詢上限 2 分鐘）/ 不存在（從 `COMPOSE_PROFILES` 拿掉）。
  三個服務可任意組合 = 八種佈局都要成立。
- Jellyfin 媒體庫：套件內預設 3 個、使用者可以改名與增刪（票 06f：剖面是一張可編輯的清單——內容類型 + 名稱 + 資料夾，照 Jellyfin 啟動精靈「新增媒體庫」；靠泊之後建好的列鎖住、標「已建立」，改名刪除去 Jellyfin，還沒建的照樣能加）；既有 1–20 個，每個 0–5 條路徑。
- 索引站：9 個預設站（AniDex 拿掉了，票 06e），逐站 pending / ok / failed；加入之後逐站試搜。
- qBittorrent 差異：0–5 個鍵。
- Route：套件內每個媒體庫一個（預設 3 個）；既有 0–20 個（使用者勾選）。
...
- **拓撲**：常駐的泊位板橫幅（五格）+ 單一工作面；第 1、2 步住板上方的前置列（§8）。沒有第二層導覽。
- **桌機**：橫幅一列五格；工作面兩欄，左剖面（將寫入什麼）／右動作。
- **手機**（PRODUCT.md：手機要能完成日常操作）：橫幅排成 2+2+1 的格子（票 06d 的 shape；不做水平捲動——
  DESIGN.md 拒絕把泊位板做成 carousel），五格同時可見；
  工作面單欄，**工作面在前、剖面在後**（票 06h 改：原本剖面在上，390 寬時每一步的標題要捲到 800–1500px
  才出現，標題順序也跳成 h1 → h3；桌機用 grid 把剖面放回左欄，看起來不變，`web/src/setup/StepFrame.tsx`）。
  剖面只放讀的東西，要填要按的放工作面（套件內 Jellyfin 的媒體庫清單因此從剖面搬進工作面）。
  主要動作固定在底部安全區之上，捲動時底部留出它的高度（`scroll-padding-bottom`）。八步全部要在手機上做得完。
