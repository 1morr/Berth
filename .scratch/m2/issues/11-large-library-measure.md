...
- [ ] 五件各有實測數字（筆數、位元組、p50 / p95），環境與資料量說得出來（1,000 部怎麼造的）
- [ ] 實驗腳本留在 `scripts/experiments/`，可重跑，起停一次性容器
- [ ] 對照門檻給出判定：超了哪一條、沒超哪一條，白紙黑字
- [ ] 超門檻的那幾支**做了分段取**，量測後 p95 回到門檻內（貼前後數字）；沒超的明說不做並說明
      為什麼不順手做
- [ ] 被刪掉的帳號那一項有結論，寫回 brief §20.8（停用與刪除兩種行為並列）
- [ ] 結論摘進 brief §20.8 並連結 `docs/research/`；推翻既有做法時同一個 commit 改文件並在
      progress.md 記一行
