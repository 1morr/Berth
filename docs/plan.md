...
- `rss_feeds`：`id`、`name`、`url`（unique）、`kind`（`mikan`；`nyaa` / `acgrip` 在票 11、`generic` 之後——由網址的主機認出來）、`interval_sec`（預設 900）、`exclude_json`（票 10）、`enabled`（沒有消費者，未建）、`last_polled_at`、`last_error`、`primed_at`（第一輪預覽決定過了沒，brief §15，票 11）、`created_at`
- `rss_series`（2026-09-24 取代 `rss_rules`，brief §15）：`id`、`key`（Mikan 是 `mikan:<番組 id>:<字幕組 id>`，其他是標題骨幹 + 字幕組；unique）、`mikan_bangumi_id`、`mikan_subgroup_id`、`title_raw`（長出它的那一筆 Item 的標題）、`group`（未建：`title_raw` 已經帶著）、`media_id`（nullable = 待綁定）、`route_id`、`season`、`episode_offset`、`exclude_json`（票 10）、`confirmed`（第一批審核確認過了沒，票 13）、`bound_by`（`system` / 使用者 id，`events.actor` 的形狀；沒綁是空字串）、`reasons_json`（票 09：自動綁定查到的 `domain.BindReason`，綁上時是依據、待綁定時是為什麼；`NULL` 是沒查過）、`candidates_json`（票 09：給人一鍵選的作品 id）、`backfilled_at`（票 12）、`enabled`（未建）、`created_at`
- `rss_items`：`id`、`feed_id`、`guid`（與 feed 合併 unique）、`title`、`link`、`torrent_url`、`info_hash`、`published_at`、`seen_at`、`release_info_json`（票 10）、`series_id`、`job_hash`（弱引用）、`status`（`unbound` 待綁定、`matched` 綁好還沒送成、`downloaded` 送出去了；`new` 與 `excluded` 在票 10）、`error`
...
| `rss_poller` | 每 30 秒醒一次，到時間的 Feed（上次輪詢滿自己的 `interval_sec`，預設 15 分鐘）各輪一次；精靈跑完之前不做事 | 抓 feed → 解析 → 新的 Item（同 Feed 的 GUID 去重）讀單集頁認出 RSS Series，找到或長出一列 → 寫 Item（Series 待綁定時 `unbound`、留著不送）→ 綁好的送 `add_download`（`trigger = rss`、`trigger_ref` = Series id，時間線的 actor 是 `rss:<id>`）。**單集頁抓不到的那一筆不寫**，下一輪再試（寫了就是見過）；Feed 抓不到記在那一列的 `last_error`；一個 Feed 爆掉不拖累其他的。送單被拒的那一筆留在 `matched` 帶著原文，下一輪再送（M3 票 08） |
...
| rss | **整組只有 admin**（`ADMIN_PREFIXES`：聚合 feed 的網址帶 token，綁定會替整個家送單）。M3 票 08：`GET /rss/feeds`（每個 Feed 帶它長出的 Item 數）、`POST /rss/feeds`（`{url, name}`，201；網址的主機不認得是 422 `feed_unsupported`、同一個網址是 409 `feed_duplicate`；不當場輪詢）、`DELETE /rss/feeds/{id}`（連它的 Item 一起刪，回 `{items}`；404 `feed_missing`）、`POST /rss/feeds/{id}/poll`（立刻輪這一個，回 `{items, series, submitted}`；抓不到 Feed 仍是 200，失敗在那一列的 `last_error`）、`GET /rss/series`（待綁定的排前面，每一列帶 `waiting`＝還沒送出的 Item 數）、`PUT /rss/series/{id}/binding`（`{media, route}`：綁定、凍結資料夾名、送出留著的 Item，回那一列加 `submitted`；404 `series_missing`、409 `series_bound` / `route_disabled`、422 `media_missing` / `route_missing` / `route_kind_mismatch`；**送單被拒不讓它失敗**）、`DELETE /rss/series/{id}/binding`（解除：還沒送的 Item 回到待綁定；已送的 Job 與資料夾名不動）、`GET /rss/items`（最近 50 筆）。拒絕是 `{reason, detail}`（`RssRefusal`）。原本的 `rss/rules` 一組隨 brief §15 的 RSS Series 改寫；Feed 的編輯（`PUT /rss/feeds/{id}`）沒有消費者，沒做。之後的票：預覽（11）、一次性連結 `POST /rss/oneshot`（18） | `rss.*` |
