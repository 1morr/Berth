...
| setup | `GET /setup/status`、`POST /setup/admin`、`POST /setup/detect`、`POST /setup/jellyfin/bootstrap`、`POST /setup/qbittorrent/apply`、`POST /setup/indexer/apply`、`POST /setup/routes/from-libraries`、`POST /setup/complete` | `setup.*`（§9） |
...
- `apply_recommended_preferences()`：`temp_path_enabled=true`、`temp_path=/data/torrent/incomplete`、`save_path=/data/torrent/complete`、`auto_tmm_enabled=true`、`category_changed_tmm_enabled=true`；只寫與現值不同的鍵。
...
- `.env.example`：`DATA_ROOT`、`CONFIG_ROOT`、`PUID=1000`、`PGID=1000`、`UMASK=022`、`TZ=Asia/Taipei`。沒有任何秘密要填。
...
- 兩個 override：`docker-compose.linux.yml`（無）與 `docker-compose.windows.yml`（僅差在註解與範例路徑），實際內容相同，分開只是為了讓新手複製貼上不用改。
- 目錄骨架由 Berth 啟動時建立：`/data/torrent/incomplete`、`/data/torrent/complete`、`/data/library/{movies,tv,anime}`。
...
[BitTorrent]
Session\DefaultSavePath=/data/torrent/complete
Session\TempPath=/data/torrent/incomplete
...
- 白名單只讓 compose 內網免密，LAN 使用者開 8080 仍要密碼；密碼由 Berth 精靈以 API `web_ui_password` 設為 Berth 管理員密碼（brief §20.7，此欄位只寫不讀）。
- `ServerDomains=qbittorrent` 讓 Berth 以容器名呼叫時通過 Host 檢查；T0.3 實測若仍被擋，改 `HostHeaderValidation=false` 並記錄。
- `DisableAutoTMMByDefault` 預設是 `true`（autoTMM 關閉），必須預置為 `false`，否則 category 的 save path 不生效（brief §20.2）。
- 已有設定檔就不碰：使用者升級或重建容器時不會被覆寫。
...
每一步都是冪等的 `services/setup.py` 命令，之後在設定頁可重跑。

1. **建立管理員**：帳號與密碼。bundle 模式下這組帳密同時成為 Jellyfin 管理員；勾選「同一組帳密也套用到 qBittorrent 與 Prowlarr 介面」（預設勾）則一併設定。
2. **偵測服務**：以 compose 主機名探測 `jellyfin:8096` `/System/Info/Public`、`qbittorrent:8080` `/api/v2/app/version`、`prowlarr:9696` `/ping`。三個都通 → `bundle`；否則 → `existing`，顯示表單填位址與憑證，每項有「測試連線」。
3. **Jellyfin**：bundle 且 `StartupWizardCompleted=false` → §9.4 全自動；existing → 以管理員帳密登入、建立 API key、列出媒體庫，並提供「安裝 MergeVersions」按鈕（需確認）。
4. **qBittorrent**：套用建議偏好（§8.1）、設定密碼（bundle）；existing 模式先顯示差異再套用。
5. **索引站**：bundle → 勾選預設公開站清單（預設全勾）：Nyaa.si、dmhy、AniDex、Anime Tosho、ACG.RIP、Mikan、1337x、YTS、EZTV、The Pirate Bay；Berth 以 `indexer/schema` 取定義、`indexer` 新增、`indexer/test` 驗證。existing → Prowlarr 位址 + API key，或任意 Torznab 端點 + key（Jackett）。
6. **TMDB**：內建專案 key，可覆寫；按「測試」。
7. **媒體庫與 Route**：bundle → 自動由三個媒體庫建立三個 Route（`movies` / `tv` / `anime`，anime 用 `anime` profile）並建立 qBittorrent category；existing → 使用者勾選媒體庫、多路徑時選寫入目標；每個 Route 立即跑硬鏈接測試。
...
- 絕不自動建立媒體庫、安裝插件或改既有媒體庫設定；這三件事各是一顆按鈕，按下前顯示將執行的 API 與參數。
- 路徑一致性檢查：向 qBittorrent 讀 `save_path` 與 category 路徑，向 Jellyfin 讀媒體庫路徑，逐一 `stat` 確認 Berth 看得到；看不到就在健康頁指出「哪個容器的哪個路徑對不上」並附 compose 修正範例。
...
| T0.6 | 設定精靈（§9.3）：`setup/*` API 與 UI；bundle 偵測；一鍵設定 Jellyfin / qBittorrent / Prowlarr；existing 模式表單；從媒體庫建 Route；Route 硬鏈接檢查 | 在乾淨的 Linux 與 Windows Docker Desktop 上，`docker compose up` 後只操作 Berth 即完成設定 |
