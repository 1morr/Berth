...
> 2026-09-24 拆成 21 張票，同日插入 06b，共 22 張；編號與內容以 `.scratch/m3/issues/` 為準：01–06b 是下方「M3 之前先做的修補」與兩個頁面決定，07 起是 RSS（tracer 是 08）。
...
- 審核頁的**批次確認**：同一個 Job 或同一個 RSS Series 的 audit 一組一顆「全部確認」（2026-09-24 使用者拍板；e2e 那一包芙莉蓮的 11 個特典目前要逐一按）；audit 段另有一顆整段的「全部確認」，只確認畫面上列出的那些（同日試跑時加，一包 12 集全是 medium）。
