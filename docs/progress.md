...
| M1.5 | `.scratch/m1.5/issues/` | plan §11.2b；brief §12、§13、§19、§20.8；`docs/research/library-browsing.md` | 2026-09-15 使用者在票 13 之後拍板加入，排在 M1 驗收後、M2 前。拆票前先定 brief §19 的待決；CONTEXT.md 的 **Inventory** 要跟著改定義 |
...
| 2026-09-15 | （規劃，無票） | 票 13 收尾後使用者拍板：媒體庫改成像 Jellyfin 那樣瀏覽，播放仍跳 Jellyfin，排成 M1.5（M1 驗收後、M2 前）。brief §1.1、§1.2、§12、§13、§17、§18、§19 與 plan §11.2b 已改，Jellyfin API 的可行性查證寫在 `docs/research/library-browsing.md`、摘進 brief §20.8 | `/implement .scratch/m1/issues/14-route-settings.md`（新 session）；M1.5 拆票前先定 brief §19 的待決 |
