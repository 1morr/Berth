...
- 2026-09-17 票 14d 發現（沒修）：`release._numbers` 在**季號等於方括號集號**時把季號丟掉——`[桜都字幕组] … S2 / Mushoku Tensei S2 [02]` 讀成沒有季號的第 2 集，是為 `The_Final_Season[28]` 寫的那一條誤傷（票 01 資料裡 5 個檔案）。這 5 個在規則 1 之下送審核；但 TMDB 併成一季的作品碰不到規則 1——開票前重掃才發現 Re:Zero `S2][02]` 會自動入錯。使用者要求開票：M1 票 14f（掃過之後發現 Re:Zero 那種 TMDB 併成一季的作品會自動入錯）。
