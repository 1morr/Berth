...
| 2026-09-24 | M2 13 前端品質（M1.5 audit P1 / P2 + P3 九條） | **1–17 條都有結果，量測前後貼在票上**：首頁 CLS 0.3453 → 0、501px 0.4400 → 0、媒體庫 0.1538 → 0（有上一次紀錄時；第一次來照舊——使用者拍板照「上一次的形狀」佔位，固定佔位只對 0 / 1 / 2 列其中一種剛好），Movies 牆 DOM 1,732 → 931、Tab 停留點 222 → 121、回應 53.7 → 27.6 KB（一頁 50 部）。`?filter=` 擋不住的原因找到了：根路由不驗網址，子路由的結果沒寫回的那一格照原樣漏下來（`page=abc`、`sort=` 也是），`validateSearch` 改成每一格都寫回。四個卡片元件的控制項與整格連結名字帶上作品名、四面牆 `<ul>` + `<h3>`、兩組分頁兩個名字、`NAV_LINK` 走 `data-status`、篩選列當前是 `aria-current="true"` 的字、集表 caption 與窄版片長播出、方向下拉的可見標籤、`TmdbAttribution`、`Poster.tsx` 併進 `ArtSlot` 並給 `srcset`（後端多 `poster_large` / `wide_large`）、`tmdbText` → `displayRound`、`/search/queries` 先帶 `not_configured`、缺集 placeholder、第 2 頁說兩列去了哪裡。**不修（理由在票的 Comments）**：Tab 停留點其餘、`ExpandHint`（2.5.3）、`<summary>` 的 role（ARIA in HTML 禁止，子代理查證）、庫存 ETag。每一條 a11y 修復斷言無障礙樹並做變異，第 2 條例外（缺陷不在無障礙樹上，比 class）。code-review：Standards 七條修掉（i18n 字串拼接改插值、註解錯位、DESIGN.md 殘留、名詞表用詞、`TEXT_LINK`、讀紀錄 hook、測試命名），Spec 抓到第 2 條只測了一處（補成三處各變異）；其餘記在票的 Comments。`/impeccable document` 刷新 `.impeccable/design.json`。前端 eslint / tsc / prettier 綠、`pnpm test` **624 passed**；ruff / mypy（270 files）/ lint-imports（6 kept）綠、`pytest` **2218 passed**；playwright 實跑探索、媒體庫（含第 2 頁）、詳情（1280 與 390） | `/implement .scratch/m2/issues/14-critique-remainder.md`；刪掉 worktree `1morr/main-3` 前先問使用者 |
