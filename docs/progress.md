...
- 2026-09-17 票 14d 發現（沒修）：`release._numbers` 在**季號等於方括號集號**時把季號丟掉——`[桜都字幕组] … S2 / Mushoku Tensei S2 [02]` 讀成沒有季號的第 2 集，是為 `The_Final_Season[28]` 寫的那一條誤傷（票 01 資料裡 5 個檔案）。規則 1 之下它們送審核，不會入錯；要不要開票由使用者決定。
