...
| `jellyfin/items.tv.series.index.json` | 同上一輪加錄。`GET /Items?userId=U&parentId=<TV>&recursive=true&includeItemTypes=Series&fields=ProviderIds&enableImages=false&enableUserData=false&enableTotalRecordCount=false`（Berth 端比對用的整份清單）：沒有 `ImageTags` 與 `UserData`，`ProviderIds` 與 `ProductionYear` 還在；**`enableTotalRecordCount=false` 不帶 `limit` 時照樣回 `TotalRecordCount`** |
