...
| `jellyfin/items.tv.series.search.json` | **M2 票 14 加錄**（`--record --only`，另一輪一次性容器：使用者 id、`ServerId`、日期與 `ImageTags` 和這一組其他檔案不同；item id 相同）。牆的查詢加 `searchTerm=show`：剩 Alpha、Bravo 與 Hotel Show 三部、仍照 `sortBy=SortName` 排，`TotalRecordCount` 是搜過之後的 3——`searchTerm` 伺服器真的有作用、搜尋不會把排序換成相關度的證據 |
