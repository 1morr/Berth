...
- **按了還在跑:** 按鈕不停用，只換文字（「搜尋」→「搜尋中…」）；說不行的是旁邊那句話或欄位本身。**送出中按了不再送**
  （M3 票 06）：`PrimaryButton` / `GhostButton` 的 `busy` 給 `aria-disabled` 並吞掉點擊（連 Enter 的隱式送出），外觀同停用態，
  但**不用 `disabled`**——停用的鍵接不住焦點，按下去的那一刻鍵盤使用者掉回 `body`（M1.5 票 05 的切換鍵先踩到）。
  `disabled` 只留給「現在不成立」的那一種（表單沒改過、一個都沒勾），那時沒有人剛按下它。
...
**精靈上走過的與目前的那幾格是按鈕**（票 06d，`onSelect`）：整格就是一顆 `<button>`，漆與底線塗在按鈕上，
`aria-current="step"` 跟著移到按鈕；還沒到的那幾格仍是純文字——前進只能靠把事做完。健康頁不給 `onSelect`，
整塊板沒有一個按鈕。

**精靈的導覽**（票 06d，`setup/BerthNav.tsx`、`SetupPage.tsx`）：板上方是**前置列**（`.label` 的「前置」+ 兩顆
`NAV_BOX` 方塊：管理員是誰、判定了幾個服務；目前那一步換 `NAV_BOX_ACTIVE`），第 1、2 步不上板。回頭看得比剛做完那一格
更前面時，板下一條 `deck` 底的帶子說出在哪裡、目前走到第幾步，右邊一顆 Ghost「回到目前這一步」。做完的那一頁在 lede
下方一塊 `well` + `rule` 框的 `role="note"`（「這裡能做 / 不在這裡做」兩列，`.label` term）。工作面最底下是
「上一個泊位」（Ghost）與「前往下一個泊位」（Primary，那一頁做完才有）：有「下一個」時它是主要動作，窄版固定在底部、
兩顆排在同一列；那一頁自己的「重跑」同時降成 Ghost、不再固定——兩個 sticky 會疊在同一個位置。
...
- **底下多一行控制項的那一種是同一個元件**（`components/Tile.tsx`，M2 票 14）：媒體庫牆的卡片與觀看區的集卡——圖與標識帶
  一條連結、底行（`min-h-10`、`border-t-2`）在連結外面並排，框的 hover 用 `has-[a:…]`。探索牆的 `MediaTile` 與接著看的
...
- **只在媒體庫頁**（M3 票 06：探索頁只放 TMDB 牆，登入後落在媒體庫；M1.5 票 07 原本在首頁搜尋列之上攤開）：在切換列與
  「還沒進 Jellyfin」之間，**收成一行「接著看 N 項」**（M2 票 14：`NAV_BOX` 小一號的開關、`aria-expanded`，展開才畫兩列、換頁面不記住；讀取中照
...
- **讀取中照這個人在這一頁上一次的形狀佔位**（M2 票 13，推翻 watching-shape 的「讀取中不畫」：資料回來才插進來，
  ...
  第一次來沒有紀錄，不佔位。沒有內容的那一列不畫。問不到 Jellyfin 時不另外說（牆會說）。
...
- **觀看（`WatchSection`）:** 區塊標題 `觀看` + 計數位置說「剩 N 集沒看 / 全部看完了」+ 靠右文字連結「在 Jellyfin 開啟」。
  ...
  只在第一次決定（標記之後主按鈕換季，眼前這一季不跳）。集是**橫放的貨櫃**，與媒體庫頁接著看的兩列同一種、同一份 `WALL_GRID`：16:9 的
...
- **由上到下四格:** ① 識別：中性色塊的類別標籤（`.label` + `deck`）+ 標題（`.value wrap-anywhere`）；② 一句話：理由翻成的
  句子 `Dot` 「多久以前」（`text-sm ink-dim`）；③ 展開：機器字串收在原生 `<details>`（摘要看得見的只有 `.label`「展開」，`min-h-6`；可存取名稱以 `aria-labelledby` 接上這一件的標題——一頁幾件就有幾顆「展開」，M3 票 06），裡面是
...
- **範圍 + 即時估算**（`JobDelete`，一個元件掛在 `/jobs/:hash` 的動作區與 Media 詳情的版本清單，確認區 `max-w-3xl`）：
  ...
  拒絕以 `role="alert"` 的 `blocked-ink` 留在區塊裡；成功時區塊收起，觸發鍵下方一行 `role="status"` 說**後端回報真的做掉了什麼**，
  不是勾選的回聲；那一格**一直掛著**、空的時候不佔間距（M3 票 06：跟著結果一起掛上去的 live region 多數閱讀器不念）。
